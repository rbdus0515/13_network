...
//		1. 서버의 포트번호 정함
		int port = 8500; // prot 번호는 0 ~ 65535 사이 지정 가능
		...
		try {
			// 2. 서버용 소켓 객체 생성
			serverSocket = new ServerSocket(port); // 서버용 소켓을 생성ㅎ하여 포트결함
			
			// 3. 클라이언트 쪽에서 접속 요청이 오길 기다림
			// - 서버용 소켓은 생성되고나면 클라이언트 요청이 오기전까지
			...
			// InputStreamReader : 문자기만 스트림과 바이트 기반 스트림 연결에 사용되는 스트림
			...
			// 사용한 스트림, 소켓 자원을 모두 반환 (close)
			 ...
			 try {
				 // 보조 스트림 close시 연결된 기반 스트림(is,os)도 같이 close됌.
				 if(pw != null) pw.close();
				 if(br != null) br.close();
